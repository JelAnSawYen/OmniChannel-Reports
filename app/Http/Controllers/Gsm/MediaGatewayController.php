...
namespace App\Http\Controllers\Gsm;

use App\Http\Controllers\Controller;
use App\Http\Controllers\HandlesBulkDestroy;
use App\Http\Controllers\HandlesInventoryImport;
use App\Http\Requests\Gsm\StoreGsmSimAssignmentRequest;
use App\Http\Requests\Gsm\StoreMediaGatewayRequest;
use App\Http\Requests\Gsm\UpdateMediaGatewayRequest;
...
class MediaGatewayController extends Controller
{
    ...
    public function bulkDestroyAssignments(Request $request, MediaGateway $mediaGateway)
    {
        abort_unless($this->isGsm(), 404);
        abort_unless($request->user()?->hasPermission('media.delete'), 403, 'You do not have permission to perform this action.');
        $this->denyStandardMutation();
        ...
        return response()->json([
            'message' => 'Selected SIM assignments deleted successfully.',
            'record' => $this->jsonRecord($mediaGateway->fresh(['assignments'])),
        ]);
    }
    ...
    public function bulkDestroy(Request $request)
    {
        ...
        foreach ($this->validatedBulkIds($request) as $id) {
            $mediaGateway = MediaGateway::query()->find($id);
            if (! $mediaGateway) {
                continue;
            }
            $code = $mediaGateway->site_code;
