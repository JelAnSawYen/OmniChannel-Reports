...
<div class="app-shell">
    <aside class="sidebar" id="sidebar" aria-label="Sidebar navigation">
        ...
        <div class="nav-section"><div class="section-label">Manage</div>
            <div class="nav-list">
                @if(auth()->user()->canAccessModule('campaigns'))<a href="{{ route('campaigns') }}" class="nav-item {{ request()->routeIs('campaigns', 'campaigns.*') ? 'active' : '' }}"><span class="nav-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m3 11 18-5v12L3 13v-2z"/><path d="M11.6 16.8a3 3 0 1 1-5.8-1.6"/></svg></span><span>Campaigns</span></a>@endif
                @if(auth()->user()->canAccessModule('pdc-servers'))<a href="{{ route('pdc-servers') }}" class="nav-item {{ request()->routeIs('pdc-servers') ? 'active' : '' }}"><span class="nav-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3" y="4" width="18" height="7" rx="2"/><rect x="3" y="13" width="18" height="7" rx="2"/><circle cx="7" cy="7.5" r=".9" fill="currentColor" stroke="none"/><circle cx="7" cy="16.5" r=".9" fill="currentColor" stroke="none"/></svg></span><span>PDC Servers</span></a>@endif
                @if(auth()->user()->canAccessModule('sip-channels'))
                <div class="nav-group{{ $sipChannelsOpen ? ' open' : '' }}" id="sipChannelsGroup">
                    <a href="{{ route('sip-channels') }}" class="nav-item nav-parent-row {{ request()->routeIs('sip-channels') ? 'active' : '' }}" id="sipChannelsToggle" aria-expanded="{{ $sipChannelsOpen ? 'true' : 'false' }}" aria-controls="sipChannelsSub">
                    ...
                    <div class="nav-sub" id="sipChannelsSub">
                        <a href="{{ route('channel-range-list') }}" class="nav-item nav-subitem {{ request()->routeIs('channel-range-list', 'channel-range-list.*') ? 'active' : '' }}"><span class="nav-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M8 6h13"/><path d="M8 12h13"/><path d="M8 18h13"/><path d="M3 6h.01"/><path d="M3 12h.01"/><path d="M3 18h.01"/></svg></span><span>Channel Range List</span></a>
                    </div>
                </div>
                @endif
                @if(auth()->user()->canAccessModule('channel-allocation'))<a href="{{ route('channel-allocation') }}" class="nav-item {{ request()->routeIs('channel-allocation', 'channel-allocation.*') ? 'active' : '' }}"><span class="nav-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg></span><span>Channel Allocation</span></a>@endif
                @if(auth()->user()->canAccessModule('archive-recordings'))<a href="{{ route('archive-recordings') }}" class="nav-item {{ request()->routeIs('archive-recordings') ? 'active' : '' }}"><span class="nav-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M13.5 3H7.5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h9a2 2 0 0 0 2-2V8l-5-5z"/><path d="M13.5 3v5h5"/><path d="M10 12.8v4l3.4-2-3.4-2z"/></svg></span><span>Archive Recordings</span></a>@endif
    ...
    <main class="main">
        ...
        <div class="flash-host" aria-live="polite">
            @if(session('success'))<div class="flash success" role="status"><span class="flash-message">{{ session('success') }}</span><button type="button" class="flash-close" aria-label="Close">×</button></div>@endif
            @if(session('error'))<div class="flash error" role="alert"><span class="flash-message">{{ session('error') }}</span><button type="button" class="flash-close" aria-label="Close">×</button></div>@endif
            @if($errors->any())<div class="flash error" role="alert"><span class="flash-message"><ul>@foreach($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul></span><button type="button" class="flash-close" aria-label="Close">×</button></div>@endif
        </div>
